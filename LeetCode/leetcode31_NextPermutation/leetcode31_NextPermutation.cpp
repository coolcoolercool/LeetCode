...
public:
	...
	void nextPermutation(vector<int> &nums) {
		int size = nums.size();
		int i = 0;
		for (i = size - 2; i >= 0; i--) { // 从后往前找到第一个相邻升序对
		...
		if (i == -1) {
			...
			for (int j = size - 1; j >= i + 1; --j) { // 从后往前[i+1,end)找第一个大于a[i+1]的值
				if (nums[i] < nums[j]) {
					swap(nums[i], nums[j]); // 交换二者
					reverse(nums.begin() + i + 1, nums.end()); // 反转[i+1,end)，变成升序. 注意这一步别少了, 这里使用sort排序也行
					break;
				}
			}
		}
