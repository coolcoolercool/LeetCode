...
核心思想:
 贪心 + 优先队列
我们首先思考，如果不限制次数下我们可以获取的最大利润，我们应该如何处理？我们只需将所有的项目按照资本的大小进行排序，依次购入项目 iii，同时手中持有的资本增加 profits[i]，直到手中的持有的资本无法启动当前的项目为止。
...
public:
	int findMaximizedCapital(int k, int w, vector<int>& profits, vector<int>& capital) {
		...
		priority_queue<int, vector<int>, less<int>> pq; // 最大堆，大根堆. top就是最大值
		vector<pair<int, int>> vec; // <资本，利润>
		...
		int curIndex = 0;
		for(int i = 0; i < k; i++) {
			while(curIndex < size && vec[curIndex].first <= w) { // 每次IPO后，资本可能会增加，可以做的IPO可能会增加
