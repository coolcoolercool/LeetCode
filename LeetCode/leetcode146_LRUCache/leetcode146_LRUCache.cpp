...
维护一个链表，链表每个节点是一对{key, value}, 从头到尾，使用最近使用到最远使用
eg:
l1 -> l2 -> l3，是l1 最近使用，l3最远使用，如果要删除，就删除l3

在维护一个map，map的{key, 指向链表k_v节点指针}, 在map中可以通过key获取到指向链表k_v的节点指针，然后间接获取key对应的value值
