...
public:
	bool canJump(vector<int> &nums) {
		int size = nums.size(), rightMost = 0;

		for (int i = 0; i < size; i++) {
			if (i <= rightMost) {  // rightMost 表示目前可以到达最远的index的位置，index以及之前的索引都可以达到
				rightMost = max(rightMost, i + nums[i]);  // 比较之前的可以到达的最远的位置和当前位置可以到达最远的位置，取较大值
